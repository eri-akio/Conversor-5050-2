...
| `Base` | `naturezaContingencia` | `naturezaContingencia` | `evento` | `str` | 2020/2025: `TRI|TRA|CIV|NA` | Tributária, trabalhista, cível ou não aplicável | Excel: obrigatória; XML: obrigatória | `TRA - Trabalhista` → `TRA`; maiúsculas | Validar domínio localmente (`BASE-NATUREZA-FORM-001`); validar coerência com avaliação, risco e contas | 12/2020 até 2026-06 | XSD 12/2020 e 06/2025 |
...
> **Nota (decisão de negócio) sobre `tipoAvaliacao = IE/ME`:** diferente da política cautelosa registrada em `CONF-002` (`docs/conflitos_documentais.md`), `IE`/`ME` são aceitos incondicionalmente (sem gating por `dataBase >= 2026-12`) e emitidos no XML tal como informados, sem bloquear o status de aprovação — mesmo o XSD 06/2025 fornecido só declarando `I|M|NA`. Decisão explícita do usuário, não uma correção técnica. As regras de negócio já restritas a `"I"`/`"M"` (`BASE-CONT-001`, `DRO001301/1302/1312/1313/1314`, `DRO000003/4/5`, cálculo de `valorTotalRisco`) permanecem inalteradas e, portanto, não se aplicam a `IE`/`ME` — inclusive `BASE-CONT-001`, que continuará rejeitando `IE`/`ME` combinado com `naturezaContingencia` real (`TRI`/`TRA`/`CIV`).
